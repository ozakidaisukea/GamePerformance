...
#define FAILE_NAME	"data\\TEXT\\ownermotion.txt"	// 読み込むtext名

//=============================================================================
// 変数
//=============================================================================
COwner::KEY COwner::m_aKeyOffset[MAX_PARTS] = {};

int	COwner::m_nNumParts = {};
int	COwner::m_nNumModel = {};
CMeshOrbit*COwner::m_mesh;
COwner::KEY_INFO*COwner::m_pKeyInfo[MAX_PARTS] = {};
COwner::MOTION_INFO COwner::m_aMotionInfo[MAX_PARTS] = {};

//=============================================================================
// 静的メンバ-変数宣言
//=============================================================================
LPDIRECT3DTEXTURE9 COwner::m_pTexture = NULL;				// テクスチャ
LPD3DXBUFFER COwner::m_pBuffMatModel[MAX_PARTS] = {};		// バッファ
DWORD COwner::m_nNumMatModel[MAX_PARTS] = {};			    // マテリアル情報の数
LPD3DXMESH COwner::m_pMeshModel[MAX_PARTS] = {};			// メッシュ情報へのポインタ
D3DXVECTOR3	COwner::m_pos = D3DXVECTOR3(0, 0, 0);			// 位置
D3DXVECTOR3 COwner::m_rot = D3DXVECTOR3(0, D3DX_PI, 0);			// 向き
char COwner::m_aFileNameModel[MAX_PARTS][256] = {};
CModel*COwner::m_apModel[MAX_PARTS] = {};
int COwner::m_aIndexParent[MAX_PARTS] = {};
COwner::OWNERSTATE COwner::m_State = COwner::OWNERSTATE_NONE;

//=============================================================================
// 読み込み
//=============================================================================
HRESULT COwner::Load(void)
{
	// デバイスを取得
...
// コンストラクタ
//=============================================================================
COwner::COwner(int nPriority, OBJTYPE objtype) :CScene(1, OBJTYPE_PLAYER)
{
	m_nCntA = 0;
	m_nCntB = 0;
	...
	m_State = COwner::OWNERSTATE_NONE;
	m_nMotionType = 0;
...
void COwner::Update(void)
{
	...
	// 前移動
	if (InputKeyboard->GetPress(DIK_W) == true || InputJoypad->GetPress(CInputjoypad::DIJS_STICK_LEFT_UP) == true)
	{
		m_pos.z += 2.0f;
	...
	// 応援
	//========================================================
	if (InputKeyboard->GetPress(DIK_RETURN) == true || InputJoypad->GetTrigger(CInputjoypad::DIJS_BOTTON_LB) == true
		|| InputJoypad->GetTrigger(CInputjoypad::DIJS_BOTTON_RB) == true)
	{
		pSound->PlaySound(pSound->SOUND_LABEL_SE_OUEN);
...
#ifdef _DEBUG
	// 範囲設定のデバック
	m_Dubug->Print("ssfsfsf", "\n[ オーナー 位置 ] ", "X", m_pos.x, "Y", m_pos.y, "Z", m_pos.z);
...
// プレイヤーの生成
//=============================================================================
COwner *COwner::Create(D3DXVECTOR3 pos,D3DXVECTOR3 rot)
{
	COwner *pPlayer = NULL;

	if (pPlayer == NULL)
	{
		pPlayer = new COwner(1);
...
// プレイヤーのポジション取得
//=============================================================================
D3DXVECTOR3 COwner::GetPlayerpos(D3DXVECTOR3 pos)
{
	m_pos = pos;
...
// プレイヤーの位置取得
//=============================================================================
D3DXVECTOR3 COwner::Getpos(void)
{
	return m_pos;
}
...
// プレイヤーの向き取得
//=============================================================================
D3DXVECTOR3 COwner::Getrot(void)
{
	return m_rot;
}
...
// プレイヤーの動き取得
//=============================================================================
D3DXVECTOR3 COwner::Getmove(void)
{
	return m_move;
}
...
void COwner::UpdateMotion(void)
{
	...
	{
		if (m_apModel[nCntModel] != NULL && m_pKeyInfo[m_nMotionType] != NULL)
		{
			// 現在のキー
	...
	if (m_pKeyInfo[m_nMotionType] != NULL)
	{
...
// 1行分(256)の読み込み
//=============================================================================
char * COwner::ReadLine(FILE *pFile, char * pDest)
{
	// 1行分(256文字分)読み込み)
	while (1)
	{// 有効な文字列が見つかるまで回す
...
// 文字列の先頭を取得
//=============================================================================
char * COwner::GetLineTop(char * pSrc)
{
	while (1)
	{// 有効な文字列が見つかるまで回す
...
// 文字列を抜き出す
//=============================================================================
int COwner::PopString(char * pSrc, char * pDest)
{
	int Word = 0;
