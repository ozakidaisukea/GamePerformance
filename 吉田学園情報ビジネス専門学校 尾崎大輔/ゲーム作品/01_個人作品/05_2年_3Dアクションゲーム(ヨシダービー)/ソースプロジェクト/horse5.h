...
public:
	...
	CHorse5(int nPriority = 1, OBJTYPE objtype = OBJTYPE_PLAYER);
	~CHorse5();
	...
	void	Draw(void);									// 描画処理
	static CHorse5 *Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, JOCKEYTYPE type);			// 生成処理
	D3DXVECTOR3 GetPlayerpos(D3DXVECTOR3 pos);			// 位置取得
	static D3DXVECTOR3 Getpos(void);					// 位置取得
	static D3DXVECTOR3 Getrot(void);					// 位置取得
