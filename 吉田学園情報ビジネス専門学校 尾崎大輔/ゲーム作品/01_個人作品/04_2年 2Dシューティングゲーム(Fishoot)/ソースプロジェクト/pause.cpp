...
HRESULT CPause::Load(void)
{
	...
	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\game\\Continue.png",
		&m_pTexture[0]);

	// テクスチャの生成[2]	リトライ
	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\game\\Retry.png",
		&m_pTexture[1]);

	// テクスチャの生成[3]	クィット
	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\game\\Quit.png",
		&m_pTexture[2]);

	// テクスチャの生成[4]	背景
	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\game\\pausebg000.png",
		&m_pTexture[3]);
...
// 初期化処理
//=============================================================================
HRESULT CPause::Init(D3DXVECTOR3 pos, float widtgh, float height)
{
	//ローカル変数
	VERTEX_2D*pVtx;
	...
	for (int nCnt = 0; nCnt < MAX_PAUSEMENU; nCnt++)
	{
		if (nCnt == 3)
		{
			m_pScene2D[nCnt] = new CScene2D(8);
		}
		else
		{
			m_pScene2D[nCnt] = new CScene2D(9);
		}
		m_pScene2D[nCnt]->CScene2D::Init(pos, widtgh, height);
...
void CPause::Update(void)
{
		...
		InputJoypad->GetTrigger(CInputjoypad::DIJS_BOTTON_B) == true)
	{
		...
		pSound->PlaySound(pSound->SOUND_LABEL_SE_SERECT);
